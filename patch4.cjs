...
const snapchatSvgOriginal = `        <svg width="40" height="40" viewBox="0 0 24 24" fill="url(#snap-grad)" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg scale-[1.1] sm:scale-[1.25]">
          <defs>
...
const snapchatSvgFixed = `        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg scale-[1.1] sm:scale-[1.25]">
          <defs>
            <linearGradient id="snap-grad" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#FFFC00" />
              <stop offset="100%" stopColor="#FFEA00" />
            </linearGradient>
            <filter id="snap-shadow" x="-10%" y="-10%" width="120%" height="120%">
              <feDropShadow dx="0" dy="1" stdDeviation="1" floodOpacity="0.3"/>
            </filter>
          </defs>
          <rect width="24" height="24" rx="6" fill="url(#snap-grad)" />
          <path d="M11.954 3.636c.205 0 .399.162.399.162.91.757 1.488 1.955 1.488 3.01v.095c0 .243.082.473.238.648.272.298.665.405 1.05.324.604-.135 1.254-.041 1.76.297.243.162.37.405.37.662 0 .432-.303.824-.805 1.054a3.174 3.174 0 0 0-1.84 2.768c-.01 1.134.61 2.066 1.487 2.376.626.216 1.341.148 1.832-.081.798-.378 1.485.459.733.918-.707.419-1.282 1.054-1.636 1.81l-.039.095c-.37 1.013-1.403 1.62-2.474 1.62-.25 0-.498-.027-.741-.095a5.538 5.538 0 0 1-3.693 3.416 1.334 1.334 0 0 1-1.026.027 5.518 5.518 0 0 1-3.8-3.443 3.327 3.327 0 0 1-.741.095c-1.07 0-2.103-.607-2.473-1.62-.014-.027-.028-.067-.042-.108-.344-.73-.902-1.35-1.584-1.755-.742-.446-.065-1.297.742-.919.49.23 1.205.297 1.831.081.876-.31 1.497-1.242 1.487-2.376a3.176 3.176 0 0 0-1.84-2.768c-.502-.23-.805-.621-.805-1.054 0-.256.126-.5.37-.662.505-.337 1.155-.432 1.76-.297.384.081.777-.026 1.05-.324.155-.175.237-.405.237-.648v-.094c0-1.054.577-2.253 1.487-3.013a2.384 2.384 0 0 1 1.012-.472c.485-.082 1.012-.041 1.455.135h.001Z" fill="#FFF" filter="url(#snap-shadow)" transform="scale(0.85) translate(2, 2)"/>
        </svg>`;

appContent = appContent.replace(snapchatSvgOriginal, snapchatSvgFixed);
fs.writeFileSync('src/App.tsx', appContent);
console.log("Patched Snapchat SVG");
