...
const spotifyBlock = `    case 'spotify':
      return (
...
const target = `    case 'pinterest':`;

if (!code.includes('id="spBaseGrad"')) {
    code = code.replace(target, spotifyBlock + "\n" + target);
    fs.writeFileSync('src/App.tsx', code);
    console.log("Patched 3D glass icons");
} else {
    console.log("Already patched 3D glass icons");
}
