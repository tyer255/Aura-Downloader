...
if (code.includes(loadingUITarget)) {
  ...
  console.log("Could not find loading UI target.");
}
      ...
      <AnimatePresence>
        {historyToast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 50, scale: 0.9 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 bg-neutral-900/90 text-white rounded-full shadow-2xl backdrop-blur-md border border-white/10 font-medium text-sm flex items-center gap-3 whitespace-nowrap"
          >
            {historyToast.includes("download") ? <Download className="w-4 h-4 animate-bounce" /> : <Info className="w-4 h-4 text-emerald-400" />}
            {historyToast}
