                            ...
                            <div className="flex-1 min-w-0 text-center sm:text-left flex flex-col justify-center">
                              ...
                              <div className="space-y-3">
                                <button 
                                  ...
                                  className={clsx(
                                    "w-full sm:w-auto px-8 py-4 rounded-2xl font-black transition-all shadow-xl flex items-center justify-center gap-3 text-sm sm:text-base uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer hover:scale-[1.02] active:scale-[0.98]",
                                    isLight 
                                ...
                                >
                                  ...
                                  {downloadingPlaylist 
                                    ? `Packaging Playlist ZIP (${playlistProgress?.percent || 0}%)` 
                                    : `Download All Playlist ZIP (${uniquePlaylistMedia.length} Tracks)`}
                                </button>
                  ...
                  {result.mediaType !== 'playlist' && (result.source === 'spotify' || (result.qualities && result.qualities.some((q: any) => q.url?.includes('spotify-resolve')))) ? (
                    <div className="w-full space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 w-full">
                        <CopyButton url={result.qualities?.[0]?.url || result.url || ''} isLight={isLight} className="w-full sm:flex-1 rounded-xl px-4 py-3 text-xs justify-center font-bold" />
                        <QRCodeButton url={result.qualities?.[0]?.url || result.url || ''} isLight={isLight} className="w-full sm:flex-1 rounded-xl px-4 py-3 text-xs justify-center font-bold" />
                      </div>
                    </div>
